...
function App() {
  ...
  const {
    ...
    userSelections, texPath, macros, rawPreamble, basePath,
    solutionRef, setShowResult, setShowSolution, setUserSelections, setTexPath,
  ...
  // === TIKZ STATE ===
  const [cachedTikZ, setCachedTikZ] = useState<Record<string, string>>({});
  const [isCompilingTikZ, setIsCompilingTikZ] = useState(false);
  const [compileProgress, setCompileProgress] = useState({ total: 0, done: 0 });
  const [zoomedContent, setZoomedContent] = useState<React.ReactNode | null>(null);

  const containerRef = useRef<HTMLDivElement>(null);

  // Test Compile State
  const [testResult, setTestResult] = useState<{ success: boolean, error?: string, path?: string } | null>(null);
  ...
  // Background TikZ Compilation Queue
  useEffect(() => {
    ...
    questions.forEach(q => {
      ...
      const explanationMatches = (q.explanation || '').match(regex) || [];

      // Also extract from options
      const optionMatches: string[] = [];
      if (q.options) {
        q.options.forEach((opt: any) => {
          const optMatches = (opt.content || '').match(regex) || [];
          optionMatches.push(...optMatches);
        });
      }

      // Also check for image blocks in immini if necessary
      // Simplified: collect from content and explanation
      [...matches, ...explanationMatches, ...optionMatches].forEach(m => {
        if (!tikzBlocks.includes(m)) tikzBlocks.push(m);
      });
    });
    ...
    const processQueue = async () => {
      ...
      setCompileProgress({ total: tikzBlocks.length, done: 0 });

      const newCache = { ...cachedTikZ };
      let doneCount = 0;

      for (const code of tikzBlocks) {
        const normalizedKey = code.replace(/\s+/g, ' ').trim();
        if (newCache[normalizedKey]) {
          doneCount++;
          setCompileProgress({ total: tikzBlocks.length, done: doneCount });
          continue;
        }

        try {
          // Send raw code, backend handles template and libraries
          // Now passing the preamble so TikZ can use custom macros
          const result = await (window as any).electronAPI.compileTikZ(code, rawPreamble);
          if (result.success && result.svgPath) {
            // In Electron, we can use local paths in <img> if we use file:// or similar
            // But usually we need to read the content or use a custom protocol.
            // For now, let's read the content to be safe and use dangerouslySetInnerHTML
            const content = await (window as any).electronAPI.readFile(result.svgPath);
            const normalizedKey = code.replace(/\s+/g, ' ').trim();
            newCache[normalizedKey] = content;
            setCachedTikZ({ ...newCache });
          }
        } catch (err) {
          console.error("TikZ Compile Error:", err);
        }
        doneCount++;
        setCompileProgress({ total: tikzBlocks.length, done: doneCount });
      }
    ...
    processQueue();
  }, [questions, cacheVersion]);

  useEffect(() => {
    (async () => {
      const settings = await (window as any).electronAPI.getSettings();
      if (settings) setTexPath(settings.texBinPath);
    })();
  }, []);

  const handleBrowseTex = async () => {
    const dir = await (window as any).electronAPI.openDirectory();
    if (dir) {
      setTexPath(dir);
      await (window as any).electronAPI.setSettings({ texBinPath: dir });
    }
  };
  ...
  const handleTestCompile = async () => {
    ...
    setTestResult(null);
    try {
      const helloWorldTikz = `\\begin{tikzpicture} \\draw[red, thick] (0,0) circle (1cm); \\node at (0,0) {Test}; \\end{tikzpicture}`;
      const result = await (window as any).electronAPI.compileTikZ(helloWorldTikz);
      setTestResult({ success: result.success, error: result.error, path: result.svgPath });
    } catch (e: any) {
  ...
  const handleClearCache = async () => {
    if (confirm("Bạn có chắc muốn xóa toàn bộ cache hình ảnh TikZ và vẽ lại từ đầu không?")) {
      await (window as any).electronAPI.clearTikZCache();
      setCachedTikZ({});
    ...
    <div className={clsx("relative w-full h-full overflow-hidden", themeStyle.bg, theme === 'dark' && "dark")}>
      ...
      >
        ...
        <div className={clsx("h-16 px-6 border-b flex items-center justify-between shrink-0 transition-colors duration-300", themeStyle.headerBg, themeStyle.headerText, "border-opacity-10 border-gray-500")}>
          <div className="flex items-center gap-6">
            ...
            {isCompilingTikZ && (
              <div className="ml-4 flex items-center gap-2 px-3 py-1 bg-blue-50 border border-blue-100 rounded-full animate-pulse">
                <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                <span className="text-[10px] font-bold text-blue-700 uppercase tracking-tighter">
                  Đang xử lý hình vẽ ({compileProgress.done}/{compileProgress.total})
                </span>
              </div>
            )}
          </div>
                          ...
                          return (
                            <div className={clsx(
                              ...
                              <div className={clsx("pl-2", isDark ? "text-amber-100" : "text-gray-800")}>
                                <LatexRenderer content={content} cachedImages={cachedTikZ} theme={theme} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />
                              </div>
                            </div>
                          );
                        }
                        ...
                        if (match) {
                          ...
                          return (
                            <>
                              ...
                              <LatexRenderer content={mainContent} cachedImages={cachedTikZ} theme={theme} basePath={basePath} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />
                            </>
                          );
                        }

                        // Fallback Default
                        return <LatexRenderer content={content} cachedImages={cachedTikZ} theme={theme} basePath={basePath} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />;
                      })()}
                    </>
                  );
                })()}
                        ...
                        <div className="absolute top-full left-0 w-full mt-4 text-center animate-in fade-in slide-in-from-top-2">
                          <div className="inline-block px-6 py-3 bg-green-100 text-green-800 rounded-xl border border-green-200 shadow-sm">
                            ...
                            <span className="font-mono text-2xl font-bold ml-2">
                              <LatexRenderer content={q.short_answer || q.correct_answer || '...'} cachedImages={cachedTikZ} theme={theme} basePath={basePath} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />
                            </span>
                          </div>
                        </div>
                      )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="pt-0.5 flex-1">
                          <LatexRenderer content={opt.content} cachedImages={cachedTikZ} theme={theme} basePath={basePath} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />
                        </div>
              ...
              {showSolution && q.explanation && (
                <div className={clsx(
                  ...
                  <div className={clsx("text-xl leading-relaxed", theme === 'dark' ? "text-gray-200" : "text-gray-800")}>
                    <StepRevealRenderer ref={solutionRef} content={q.explanation} isOpen={showSolution} isActive={true} cachedImages={cachedTikZ} theme={theme} basePath={basePath} macros={macros} onZoom={setZoomedContent} fileRef={fileRef} />
                  </div>
                </div>
              )}
              <AnnotationLayer
      ...
      <LatexSettingsModal isOpen={showSettings} onClose={() => setShowSettings(false)}>
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-4">
          <h3 className="text-xs font-bold text-amber-800 dark:text-amber-200 mb-3 uppercase tracking-wider flex items-center gap-2">
            <span>🛠️</span> Công cụ Debug & Môi trường
          </h3>

          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">TeX Bin Path</label>
              <p className="text-[10px] text-gray-500 mb-2">Thư mục chứa pdflatex.exe (MiKTeX/TeXLive)</p>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={texPath}
                  readOnly
                  className="flex-1 px-3 py-1.5 border rounded bg-white text-gray-600 text-xs font-mono"
                  placeholder="Chưa chọn đường dẫn..."
                />
                <button
                  onClick={handleBrowseTex}
                  className="px-3 py-1.5 bg-indigo-600 text-white rounded hover:bg-indigo-700 font-bold text-xs shrink-0"
                >
                  Browse...
                </button>
              </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-amber-200/50">
              <button
                ...
                disabled={isTesting || !texPath}
                className="px-3 py-1.5 bg-green-600 text-white rounded hover:bg-green-700 font-bold text-xs disabled:opacity-50 flex items-center gap-2"
              >
                {isTesting ? 'Đang test...' : 'Test Compile TikZ'}
              </button>
              <button
                onClick={handleClearCache}
                className="px-3 py-1.5 bg-amber-600 text-white rounded hover:bg-amber-700 font-bold text-xs flex items-center gap-2"
              >
                <span>Clear Cache ({Object.keys(cachedTikZ).length})</span>
              </button>
            </div>

            {testResult && (
              <div className={clsx("mt-2 p-2 rounded text-xs font-mono whitespace-pre-wrap overflow-x-auto max-h-32 border", testResult.success ? "bg-green-50 text-green-800 border-green-200" : "bg-red-50 text-red-800 border-red-200")}>
